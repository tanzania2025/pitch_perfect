...
def load_meld_splits(data_dir, train_csv="train_sent_emo.csv",
                     dev_csv="dev_sent_emo.csv", test_csv="test_sent_emo.csv"):
...
def tokenize_datasets(ds, tokenizer, max_length=128):
    ...
    keep_cols = ["label"]
    return ds.map(tok, batched=True, remove_columns=[c for c in ds["train"].column_names if c not in keep_cols])

def compute_class_weights(train_df):
    counts = Counter(train_df["label"].tolist())
    total = sum(counts.values())
    weights = torch.tensor([total / (len(CLASSES) * counts[i]) for i in range(len(CLASSES))], dtype=torch.float)
    return weights
