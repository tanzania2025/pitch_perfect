...
meld-to-wav: ## Convert MELD MP4 files to WAV format
	python -c "from pitchperfect.data.meld_loader import process_meld_split; print('Converting train split...'); train_pairs = process_meld_split(split='train', mode='batch'); print(f'Train: {len(train_pairs)} files converted'); print('Converting dev split...'); dev_pairs = process_meld_split(split='dev', mode='batch'); print(f'Dev: {len(dev_pairs)} files converted'); print('Converting test split...'); test_pairs = process_meld_split(split='test', mode='batch'); print(f'Test: {len(test_pairs)} files converted'); print('✅ All MELD splits converted to WAV')"
