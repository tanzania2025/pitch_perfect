...
meld-to-wav: ## Convert MELD MP4 files to WAV format
	python -c "from pitchperfect.data.meld_loader import prepare_meld_split; print('Converting train split...'); train_pairs = prepare_meld_split(split='train'); print(f'Train: {len(train_pairs)} files converted'); print('Converting dev split...'); dev_pairs = prepare_meld_split(split='dev'); print(f'Dev: {len(dev_pairs)} files converted'); print('Converting test split...'); test_pairs = prepare_meld_split(split='test'); print(f'Test: {len(test_pairs)} files converted'); print('✅ All MELD splits converted to WAV')"
