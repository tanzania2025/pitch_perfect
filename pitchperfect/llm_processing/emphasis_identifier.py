...
class EmphasisIdentifier:
    ...
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.power_words = emphasis_config.get('power_words', {
        ...
        self.emotion_words = emphasis_config.get('emotion_words', {
    ...
    def identify(self, text: str, sentiment: Optional[Dict] = None) -> List[EmphasisWord]:
        ...
        for i, word in enumerate(words):
            ...
            if re.match(r'\d+\.?\d*%?', word):
                emphasis_points.append(
                    EmphasisWord(word, i, 'medium', 'number')
                )
        ...
        for pos, trans_word in transitions:
            emphasis_points.append(
                EmphasisWord(trans_word, pos, 'light', 'transition')
            )
    ...
    def _check_power_words(self, word_clean: str, position: int, original: str) -> Optional[EmphasisWord]:
    ...
    def _check_emotion_words(self, word_clean: str, position: int,
                            original: str, sentiment: Dict) -> Optional[EmphasisWord]:
    ...
    def to_simple_format(self, emphasis_words: List[EmphasisWord]) -> List[Tuple[int, str, str]]:
