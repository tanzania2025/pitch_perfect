...
class TextImprover:
    ...
    def improve(self, text: str, issues: Issues, sentiment: Dict,
                target_style: str = 'professional') -> str:
        """
    ...
    def _build_prompt(self, text: str, issues: Issues,
                     sentiment: Dict, style: str) -> str:
    ...
    def _call_openai(self, prompt: str) -> str:
        ...
        try:
            response = self.client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "You are a speech improvement expert."},
                    {"role": "user", "content": prompt}
                ],
                temperature=self.temperature,
                max_tokens=self.config.get('llm_processing', {}).get('openai', {}).get('max_tokens', 500)
            )
