...
class ProsodyCalculator:
    ...
    def calculate(self,
                  ...
                  issues: List[str],
                  sentiment: Optional[Dict] = None) -> ProsodyAdjustment:
        ...
        if rate_data['adjustment_needed']:
            adjustment.adjustment_reasons.append(f"Adjust rate to {rate_data['target_wpm']:.0f} wpm")
        ...
        if volume_data['adjustment_needed']:
            adjustment.adjustment_reasons.append(f"Adjust volume by {volume_data['db_change']:.1f}dB")
    ...
    def _calculate_pitch(self, features: Dict, issues: List[str], sentiment: Dict) -> Dict:
