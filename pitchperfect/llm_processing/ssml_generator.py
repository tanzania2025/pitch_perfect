...
class SSMLGenerator:
    ...
    def generate(self,
                 text: str,
                 prosody_guide: Dict,
                 emphasis_words: List[EmphasisWord] = None) -> str:
    ...
    def _apply_emphasis(self, content: str, emphasis_words: List[EmphasisWord]) -> str:
        ...
        for emp in sorted(emphasis_words, key=lambda x: x.position, reverse=True):
            if 0 <= emp.position < len(words):
                level = {'light': 'reduced', 'medium': 'moderate', 'strong': 'strong'}[emp.level]
                words[emp.position] = f'<emphasis level="{level}">{words[emp.position]}</emphasis>'
    ...
    def _format_special(self, content: str) -> str:
        ...
        # Percentages
        content = re.sub(r'(\d+)%', r'<say-as interpret-as="percentage">\1%</say-as>', content)
