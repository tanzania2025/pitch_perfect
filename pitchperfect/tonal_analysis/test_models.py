#!/usr/bin/env python3
"""
Optimized RAVDESS Trainer - Enhanced version that saves ALL best models
No CREMA-D combination, enhanced features, data augmentation, longer training
NOW SAVES BOTH SINGLE MODELS AND ENSEMBLES
"""
import pandas as pd
import numpy as np
import librosa
...
class OptimizedRAVDESSTrainer:
    ...
    def save_single_model(self, model, accuracy, config, experiment_name):
        """Save a single model with all necessary components"""
        model_filename = f"best_{experiment_name}_model.pth"
        ...
        torch.save({
            ...
            'input_size': 65  # Our optimized feature count
        }, model_filename)

        print(f"✅ Saved single model: {model_filename} (Accuracy: {accuracy:.1f}%)")
    ...
    def train_optimized_ensemble(self, X_train, X_val, y_train, y_val, config, n_models=5):
        ...
        # Save ensemble
        ensemble_filename = f"best_{config['name']}_ensemble.pth"
        torch.save({
        ...
        print(f"✅ Saved ensemble: {ensemble_filename} (Accuracy: {ensemble_accuracy:.1f}%)")
    ...
    def run_optimized_experiment(self, config):
        """Run optimized experiment with proper model saving"""
        print(f"\n{'='*50}")
        ...
        if config.get('use_ensemble', False):
            ...
            # Train single model AND SAVE IT
            model, final_acc = self.train_optimized_model(X_train, X_val, y_train, y_val, config)

            # ✅ NEW: Always save single models
            saved_filename = self.save_single_model(model, final_acc, config, config['name'])
...
def main():
    """Run final optimized experiments with proper model saving"""
    ...
    print("SAVED MODELS:")
    print("=" * 50)
    print("Check for these files:")
    print("- best_optimized_features_model.pth")
    print("- best_with_augmentation_model.pth  ⭐ (Your 68.9% champion!)")
    print("- best_final_ensemble_ensemble.pth")
